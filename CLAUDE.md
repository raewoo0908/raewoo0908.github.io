...
위 두 검사는 "둘 다 손댔느냐"만 본다. 짝을 같이 커밋해도 **en이 ko를 못 따라가는 drift**(문장 누락 · en에만 추가 · 의미·강도 차이)는 남는다. `check-drift.mjs`가 그걸 잡는다. **고치는 쪽은 언제나 `en.md`다.**

**사용자가 `ko.md`를 손으로 고쳤을 때** (Claude의 Edit이 아니라 에디터에서)

1. PostToolUse 훅은 **이걸 못 잡는다.** 대신 턴이 끝날 때 **Stop 훅**이 "ko는 바뀌었는데 en은 그대로"를 잡아 턴 종료를 막는다.
2. 그러면 하던 요청이 무엇이든 **`en.md`를 ko에 맞춘다** — `git diff -- <글폴더>/ko.md`로 무엇이 바뀌었는지 보고 반영하고, 블록이 어긋났는지는 무료인 `node scripts/drift-blocks.mjs <글폴더>`로 확인한다.
3. 사용자가 직접 커밋하면 `check-bilingual`이 막으며 "en.md를 갱신하세요"를 띄운다. 그 요청이 오면 2번과 같다.

> ⚠️ **세션 중에 유료 의미 검사(`check-drift`·`npm run drift`·`/fix-drift`)를 스스로 부르지 않는다.** 호출당 ~$0.5이고, `--worktree`는 지금 편집과 무관한 미커밋 초안까지 훑어 남의 글 값까지 물린다. 의미 검사는 **커밋할 때 pre-commit이 `--staged`로 한 번만** 돌린다. 세션 중 확인이 필요하면 무료인 `drift-blocks.mjs`를 쓴다. *사용자가 명시적으로 `/fix-drift`나 `npm run drift`를 요청하면 그때는 돌린다.*

> 이 게이트는 "**빠뜨릴 수 없게**"이지 "저절로 된다"가 아니다 — 대화·커밋이라는 트리거가 필요하니 **ko만 바뀐 상태를 보면 먼저 알리고 맞출 것.**

**커밋할 때**

1. `git commit` — **ko·en 중 하나라도** 스테이징되면 검사가 돈다(나머지는 인덱스에서 읽는다). 글을 안 건드린 커밋은 영향 0, 이미 통과한 내용이면 캐시로 지나간다.
2. **일치하면** 통과하고, 그 내용이 캐시에 남아 다음 커밋은 공짜다.
3. **어긋나면** 커밋이 중단되고 판정이 `.git/ko-en-drift.json`에 저장된다. **이 파일이 있다는 것 자체가 "지금 막혀 있다"는 신호**다(통과하면 지워진다).

> 오타 교정처럼 **의미가 안 바뀌는 ko 수정**이라면 `npm run drift`를 통과시켜 en 갱신 없이 풀 수 있다(그 기록이 `check-bilingual`도 함께 풀어준다).

**막혔을 때 — 최종 커밋까지**

1. Claude 세션에서 **`/fix-drift`** 실행. 리포트를 읽어 `en.md`만 고치고 재검사한다. `ko.md`는 건드리지 않는다.
2. 재검사에서 새 건이 나오면 다시 고친다 — 판정이 틀려서가 아니라 en을 고치면 비교 대상 자체가 바뀌기 때문이다. **3라운드까지**만이고, 상한에 닿으면 남은 건을 경고로 알리고 통과시킨다(**그 상태로 배포된다**).
3. 통과하면 `git add` 후 커밋. 재판정 없이 **캐시 적중으로 즉시 통과**한다. `en.md` 단독 변경이어도 `check-bilingual`이 막지 않는다 — drift가 직접 검증됐으므로 "짝을 함께 고쳐라"가 면제된다.

- **구조 위반**(링크·이미지·표·코드블록·frontmatter 불일치)은 결정적이라 **유예가 없다** — 고쳐야만 통과하고, LLM도 안 부른다.
- **ko/en이 달라야 맞는 지점**(예: 한/영 포트폴리오가 별도 문서라 링크가 다름)은 `i18n-intentional` 마커로 검사에서 뺀다. **ko·en 양쪽에 다 달아야 한다** — 한쪽만 빼면 반대쪽 링크가 홀로 남아 어차피 걸린다. 표시한 지점은 의미 검사 프롬프트에도 실려 LLM도 다시 잡지 않는다.
  ```markdown
  - 포트폴리오: [Notion](https://…) <!-- i18n-intentional(links): 한/영이 별도 문서 -->  ← 그 줄만
  <!-- i18n-intentional(links): 아래 목록은 언어별로 다릅니다 -->                        ← 다음 블록 전체
  ```
  범위는 `links`·`images`이고 **괄호를 생략하면 둘 다**. 범위 이름을 틀리면 조용히 무시하지 않고 커밋을 막는다(표시했다고 믿었는데 안 먹는 게 제일 나쁘다). 줄 단위라 같은 블록의 다른 링크는 그대로 검사된다.
- 의미 검사는 **호출당 약 $0.5**(Opus 5). `claude` CLI 부재·네트워크 실패는 경고만 하고 통과한다(fail-open). 조절 손잡이는 `check-drift.mjs`의 `BLOCK_KINDS`(기본은 `diverged`까지 전부라 뉘앙스 차이로도 막힌다)와 `DRIFT_MAX_ROUNDS`(기본 3).

...
- **연속 3회 막히면 유예한다.** LLM 판정은 en을 고칠 때마다 비교 대상이 바뀌어 원리상 수렴이 보장되지 않는다(N라운드의 수정이 N+1라운드의 `diverged`를 만들 수 있다). 그래서 폴더별 연속 차단 횟수를 캐시에 세고, `DRIFT_MAX_ROUNDS`(기본 3)에 닿으면 **LLM을 아예 안 부르고($0) 남은 건을 경고로만 알린 뒤 통과**시킨다. 통과하면 카운터는 리셋된다. **구조 검사는 결정적이라 진동하지 않으므로 유예 대상이 아니다** — 링크·이미지·표가 어긋난 건 몇 번을 돌려도 막힌다.

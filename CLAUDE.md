...
1. `git commit` — **ko·en 중 하나라도** 스테이징되면 검사가 돈다(나머지 한쪽은 인덱스에서 읽는다). 글을 안 건드린 커밋은 영향 0이고, 이미 통과한 내용이면 캐시에 걸려 그냥 지나간다.
...
> 단 **한쪽만 스테이징하면 `check-bilingual`이 먼저 막아** drift 검사까지 가지 않는다. `ko.md`만 고쳤다면 `en.md`를 갱신하는 게 정답이고, 의미가 안 바뀌는 수정(오타 등)이라면 `npm run drift`를 통과시켜 풀 수 있다.

...
> 💡 간격 단위는 초(`s`)·분(`m`)·시(`h`)·일(`d`)입니다. 단 **최소 단위는 1분**이에요. `30s`를 줘도 1분으로 올림됩니다. 왜 그런지는 [함정 모음](#️-함정-모음)에서 다시 짚겠습니다.  <!-- i18n-intentional(links): 목차 앵커는 헤딩 텍스트에서 나오므로 한/영이 다를 수밖에 없습니다 -->
...
- 반복 태스크는 예정 시각보다 **최대 30분 늦게** 발화할 수 있습니다(1시간보다 짧은 간격이면 그 간격의 절반까지). 매시 `:00` 잡이 `:30`에 뜰 수도 있다는 뜻입니다.

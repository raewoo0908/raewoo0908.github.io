...
> 💡 **바이트코드 · 디스어셈블이란?**
>
> CPython은 소스를 기계어가 아니라 **바이트코드**(파이썬 가상 머신용 명령어 열)로 컴파일해 두고 한 줄씩 실행합니다. `dis` 모듈은 그 바이트코드를 사람이 읽을 수 있는 명령어 이름으로 다시 풀어서 보여줍니다 — 어셈블리를 거꾸로 읽는다고 해서 **디스어셈블**입니다.

아래는 이미 컴파일된 바이트코드를 `dis` 모듈로 디스어셈블한 결과입니다. 
...
이제 `STORE_NAME 0` 명령어는 code object 영역의 `co_names[0]`과 frame 영역의 `평가 스택`의 pop 값을 네임스페이스에 쓰는 역할을 합니다. 네임스페이스는 내부적으로 딕셔너리이거나 커스텀 매핑일 수 있습니다. 
```python
...
`STORE_FAST 0`, `LOAD_FAST 1` — **이름이 사라지고 정수 인덱스만 남았습니다.** 컴파일 시점에 함수 안의 지역 이름을 *('변수'가 아니라 '이름'입니다. 기억하시죠?)* 전부 세어서 `co_varnames`에 순서대로 박아두고, 런타임에는 프레임에 딸린 배열의 **슬롯 번호**로 접근합니다. 실행 중에 `'x'`라는 문자열은 단 한 번도 등장하지 않습니다.
...
> ```
>
> 첫 번째는 **컴파일조차 되지 않고**, 두 번째는 실행은 되지만 `z`라는 지역 이름이 끝내 생기지 않습니다. 함수의 지역 이름은 `def`를 컴파일하는 순간 개수가 못박히고, 그 뒤로는 늘어날 방법이 없다는 뜻입니다. wild card(`*`) import는 이름이 몇 개 늘지 컴파일러가 알 수 없습니다. 그냥 'math'라는 모듈을 사용한다고 `IMPORT_NAME` opcode로 박아놓는 것뿐이지(wild card import라면 그 뒤로 `CALL_INTRINSIC_1(INTRINSIC_IMPORT_STAR)`가 따라붙습니다), 구현, 즉 그 안에 어떤 이름·함수가 있는지는 런타임에 가봐야 알 수 있습니다. Python 확장모듈이 아닌, 순수 파이썬 모듈은 처음 import되는 그 시점에 컴파일되고, 그 결과는 `__pycache__`에 `.pyc`로 남아 다음 import부터는 그 캐시를 씁니다. 그래서 `f` 함수를 컴파일하는 시점에는 import 대상 모듈의 이름들을 알 수 없는 거죠. 
>
...
어느 쪽이든 **저장되는 건 객체를 가리키는 포인터**라는 점은 똑같습니다. 저장 방식이 딕셔너리든 배열 슬롯이든, 이름이 들고 있는 것은 늘 같다는 뜻입니다.

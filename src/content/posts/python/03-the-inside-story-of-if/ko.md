...
이번 글에서는 `if`가 어떻게 구현되어 있는지 소스코드까지 내려가서 if문의 비밀을 파헤쳐보겠습니다. 
...
>
> 그래서 `range`는 `__bool__`을 따로 두고, 객체를 만들 때 미리 계산해 둔 `length` 필드가 0인지만 봅니다. 이 `length`는 파이썬 정수 객체라 크기 제한이 없고 `__bool__`은 그 참·거짓만 물으므로, `Py_ssize_t`로 변환할 일이 아예 없습니다. 아무리 큰 범위여도 안전한 이유죠. `__bool__`이 `__len__`보다 **먼저** 검사되는 순서가 여기서 그 효과를 보는 겁니다. 참 정교하지 않나요?
>
...
아주 멋있는 최적화죠? 요약하면, 들어가며 절의 예시들은 이렇게 나뉩니다.
...
> `if 2`의 경우에는 `int` 클래스의 `nb_bool`(`__bool__`)이 `True`를 리턴하기 때문에 통과하는 겁니다. *물론 컴파일러가 상수로 취급해서 TO_BOOL을 호출하지도 않긴 하지만요 😂.*

...
- **오차의 획일화**
  ...
  > 이건 마치 성격이 아주 더러운 교수님이 0점을 맞은 학생이나 59점을 받은 학생이나 똑같이 F를 주는 것과 마찬가지입니다.
...
ReLU 함수는 1969년에 일본의 컴퓨터 과학자 후쿠시마 쿠니히코 교수가 처음 논문에서 발표했으나, 본격적으로 딥러닝에 도입된 것은 2012년 **제프리 힌튼(Geoffrey Hinton) 교수 팀**이 이미지 인식 경진대회(ILSVRC)에서 <strong>알렉스넷(AlexNet)</strong>에 ReLU를 탑재하고 압도적 우승을 차지한 이후입니다. 기존 활성화 함수보다 학습 속도가 6배나 빠르면서 기울기 소실 문제를 해결할 수 있다는 점이 증명되면서 전 세계 인공지능에서 **표준적인 활성화 함수**로 자리잡았습니다.
...
class P04XORMLP(nn.Module):
    def __init__(self):
        super().__init__()
        self.hidden = nn.Linear(2, 16) # 입력 데이터 차원 2, 출력 데이터 차원이 16인 선형변환을 수행하는 Fully Connected Layer를 생성
        self.output = nn.Linear(16, 1) # 출력 차원은 1이 되어야 함. 

...
라운드마다 새 건이 나올 수 있다. 판정이 틀려서가 아니라, en 을 고치면 그 자체가 새로운
비교 대상이 되는 데다 차단 기준이 `diverged` 까지 포함해 빡빡하기 때문이다. 호출 1회에
약 0.5달러가 든다.

**같은 폴더로 3라운드를 넘기지 말 것.** 3회 연속 막히면 스크립트가 알아서 유예로 전환해
경고만 남기고 통과시키지만(`DRIFT_MAX_ROUNDS`), 거기까지 가기 전에 남은 건을 사용자에게
보여주고 판단을 받는 편이 낫다 — 기계적 상한은 돈이 새는 걸 막는 안전망이지 목표가 아니다.

여기서 통과시켜 두면 이어지는 커밋은 캐시에 걸려 공짜로 지나간다.
`en` 만 바뀐 커밋을 `check-bilingual` 이 막지 않는 것도 이 통과 기록 덕분이다.
